...
## -----------------------------------------------------------------------------
listofpackages <- c("readxl", "dplyr", "ggplot2", "zoo", "reshape2", "tidyverse",#Import relevant packages 
                    "xts", "openxlsx", "lubridate", "tibble", "patchwork", "purrr", "stringr", "grid", "gridExtra", "gtable",
                    "systemfit", "broom", "tidyr", "xtable", "kableExtra", "plm")
...
saveRDS(results, file = "model2_results.rds")
...
col_names <- c(paste0("beta_", 0:7), "adj.R^2")

printable_results <- data.frame(matrix(NA, nrow = 30, ncol = 9, dimnames = list(NULL, col_names)))
...
for (i in 1:length(countries)) {
  ...
  printable_results[coeffs_row_index, 1:8] <- results_df$Estimate[start_index:(start_index + 7)]
}
...
for (i in 1:length(countries)) {
  ...
  printable_results[se_row_index, 1:8] <- results_df$Std.Error[start_index:(start_index + 7)]
}
...
for (i in 1:length(countries)) {
  ...
  printable_results[pvalue_row_index, 1:8] <- results_df$Pr[start_index:(start_index + 7)]
}
...
for (i in 1:length(countries)) {
  ...
  se_row_index <- which(rownames(printable_results) == paste0("s.e._", country))
  printable_results[se_row_index, 9] <- adjusted_r_squared_value
}
## -----------------------------------------------------------------------------
######################### PRINT THE RESULTS IN LATEX ###########################

colnames(printable_results) <- c("$\\beta_0$", "$\\beta_1$", "$\\beta_2$","$\\beta_3$","$\\beta_4$","$\\beta_5$","$\\beta_6$","$\\beta_7$","$R^2$")
...
# 
# writeLines(fullLatexDoc, "Table2nopanelrestriction.tex")
# 
...
restrict<-c("bg_yield_s_bg_lag-es_yield_s_es_lag=0","bg_yield_s_bg_lag-fn_yield_s_fn_lag=0", "bg_yield_s_bg_lag-fr_yield_s_fr_lag=0",
            ...
            "bg_us_corp_spread_lag-oe_us_corp_spread_lag=0", "bg_glob_sp1_bg_lag-es_glob_sp1_es_lag=0","bg_glob_sp1_bg_lag-fr_glob_sp1_fr_lag=0",
            "bg_glob_sp1_bg_lag-ir_glob_sp1_ir_lag=0", "bg_glob_sp1_bg_lag-nl_glob_sp1_nl_lag=0",
            "bg_glob_sp1_bg_lag-pt_glob_sp1_pt_lag=0","bg_glob_sp1_bg_lag-fn_glob_sp1_fn_lag=0",
            "bg_glob_sp1_bg_lag-gr_glob_sp1_gr_lag=0","bg_glob_sp1_bg_lag-it_glob_sp1_it_lag=0",
            "bg_glob_sp1_bg_lag-oe_glob_sp1_oe_lag=0","bg_glob_sp2_bg_lag-es_glob_sp2_es_lag=0","bg_glob_sp2_bg_lag-fr_glob_sp2_fr_lag=0",
            "bg_glob_sp2_bg_lag-ir_glob_sp2_ir_lag=0", "bg_glob_sp2_bg_lag-nl_glob_sp2_nl_lag=0",
            "bg_glob_sp2_bg_lag-pt_glob_sp2_pt_lag=0","bg_glob_sp2_bg_lag-fn_glob_sp2_fn_lag=0",
            "bg_glob_sp2_bg_lag-gr_glob_sp2_gr_lag=0","bg_glob_sp2_bg_lag-it_glob_sp2_it_lag=0",
            "bg_glob_sp2_bg_lag-oe_glob_sp2_oe_lag=0")
...
for (country in countries) {
  ...
  formula_str <- sprintf("%s ~ %s + %s + %s + %s + %s + %s + %s",
                         y1_var, x1_var, x2_var, x3_var, x4_var, x5_var, x6_var, x7_var)
...
texFileName <- "Table2panelrestriction.tex"
fullFilePath <- file.path(nestedFolderPath, texFileName)
