...
panel restrictions, reproduces the paper's Figures 1–6 and the upper panel of Figure 8, and computes the GVAR
out-of-sample forecasts in memory.
...
|---|---|---|
| 1 | `Exploratory_Analysis.R` | Figures 1–4: co-movement of spreads vs. real variables |
...
| 3 | `d_Yields_1990_2011.R` | Business-day yields 1990–2011 |
| 4 | `Monthly_Dataset.R` | Monthly stacked dataset used in the estimation |
| 5 | `Efficient_Global_variables.R` | GVAR "global" variables, built with inverse-distance weights based on deficit- and debt-to-GDP differences across countries |
...
| 10 | `GVAR_forecasting.R` | Out-of-sample forecasts with the GVAR (kept in memory; Figures 7.3–7.4 were produced in EViews) |
...
`d_CDS_Premia_10Y_&_5Y`, `d_CDS_Premia_10Y_Newdata`, `d_Benchmark_10Y_RY_&_Swaps`, `d_Exchange_rates`,
`d_US_corporate_AAA_BAA`, `d_EIB_10Y`, `d_deficit_gdp_exp`, `d_debt_gdp_exp`, `d_Yields_1990_2011`, `y_pop_raw`,
`q_gdp_with_spain`). It is Prof. Favero's material and is **not redistributed here**:
...
`systemfit`, `broom`, `xtable`, `kableExtra`, `plm`, `MASS`, …), plus `rstudioapi` if you run it from RStudio.
Rendering Tables 1–2 to PDF requires `pdflatex` on the `PATH` (the table scripts call it directly).
...
checks for selected intermediate datasets) and `Database.RDS/`.
...
regenerating Figures 1–6 and the upper panel of Figure 8, Tables 1–2 (`.tex` and PDF) and the intermediate datasets. Only
